async function moduleInitFunction(requireAsyncModule,exports={}){function tokenBase(stream,state){var ch;// get next character
// string
...
var pFloat="(?:\\.\\d+|\\d+\\.\\d*|\\d+)",pPrecision="(?:`(?:`?"+pFloat+")?)",reBaseForm=new RegExp("(?:"+"(?:\\d+)"+"(?:\\^\\^"+"(?:\\.\\w+|\\w+\\.\\w*|\\w+)"+pPrecision+"?(?:\\*\\^[+-]?\\d+)?))"),reFloatForm=new RegExp("(?:"+pFloat+pPrecision+"?(?:\\*\\^[+-]?\\d+)?)"),reIdInContext=/(?:`?)(?:[a-zA-Z\$][a-zA-Z0-9\$]*)(?:`(?:[a-zA-Z\$][a-zA-Z0-9\$]*))*(?:`?)/;// regular expressions
return exports.mathematica={name:"mathematica",startState:function(){return{tokenize:tokenBase,commentLevel:0}},token:function(stream,state){return stream.eatSpace()?null:state.tokenize(stream,state)},languageData:{commentTokens:{block:{open:"(*",close:"*)"}}}},{exports:exports}.exports}